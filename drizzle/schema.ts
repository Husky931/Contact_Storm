...
export const domainSearchTerms = pgTable(
    "domain_search_terms",
    {
        ...
        firstSeen: timestamp("first_seen", { mode: "string" })
            .defaultNow()
            .notNull()
    },
    (table) => [
        index("idx_search_term_domain").using(
            "btree",
            table.domain.asc().nullsLast().op("text_ops")
        ),
        uniqueIndex("unique_domain_search_term").using(
            "btree",
            table.domain.asc().nullsLast().op("text_ops"),
            table.searchTerm.asc().nullsLast().op("text_ops")
        ),
        foreignKey({
...
export const domains = pgTable(
    ...
    (table) => [
        index("idx_contact_status").using(
            "btree",
            table.contactStatus.asc().nullsLast().op("text_ops")
        ),
        index("idx_domain").using(
            "btree",
            table.domain.asc().nullsLast().op("text_ops")
        ),
        index("idx_message_type").using(
            "btree",
            table.messageType.asc().nullsLast().op("text_ops")
        ),
        index("idx_submitted").using(
            "btree",
...
export const contactAttempts = pgTable(
    ...
    (table) => [
        index("idx_contact_attempt_at").using(
            "btree",
            table.attemptedAt.asc().nullsLast().op("timestamp_ops")
        ),
        index("idx_contact_attempt_domain").using(
            "btree",
            table.domain.asc().nullsLast().op("text_ops")
        ),
        index("idx_contact_attempt_status").using(
            "btree",
...
export const seoReports = pgTable(
    "seo_reports",
    {
        ...
        analysisSuccessful: boolean("analysis_successful")
            .default(false)
            .notNull(),
    ...
    (table) => [
        index("idx_seo_report_analyzed_at").using(
            "btree",
            table.analyzedAt.asc().nullsLast().op("timestamp_ops")
        ),
        index("idx_seo_report_domain").using(
            "btree",
            table.domain.asc().nullsLast().op("text_ops")
        ),
        index("idx_seo_report_overall_score").using(
            "btree",
            table.overallScore.asc().nullsLast().op("float4_ops")
        ),
        foreignKey({
            columns: [table.domain],
            foreignColumns: [domains.domain],
